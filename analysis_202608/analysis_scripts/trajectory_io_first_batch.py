...
def residue_range_heavy_atoms(topology, chain_id: str, lo: int, hi: int) -> np.ndarray:
    return atom_indices(
        topology,
        lambda atom: (
            atom.residue.chain.chain_id == chain_id
            and lo <= atom.residue.resSeq <= hi
            and atom.element is not None
            and atom.element.symbol != "H"
        ),
    )
...
def base_heavy_atoms(topology, chain_id: str, resseq: int) -> np.ndarray:
    backbone = {
...
def centroid_distance_A(xyz_A: np.ndarray, atom_ids_a: np.ndarray, atom_ids_b: np.ndarray) -> np.ndarray:
    center_a = xyz_A[:, atom_ids_a].mean(axis=1)
    center_b = xyz_A[:, atom_ids_b].mean(axis=1)
    return np.linalg.norm(center_a - center_b, axis=1)
